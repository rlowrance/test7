...
class FundamentalsOLD(AttributeMaker):
...
class HistoryOasspread(AttributeMaker):
...
class HistoryPrice(AttributeMaker):
    def __init__(self, history_length):
...
class HistoryQuantity(AttributeMaker):
    def __init__(self, history_length):
...
class HistoryQuantityWeightedAverageSpread(AttributeMaker):
...
class InterarrivalTime(AttributeMaker):
    def __init__(self):
...
class OrderImbalance(AttributeMaker):
...
class SecurityMaster(AttributeMaker):
...
class AmtOutstandingHistory(AttributeMakerStub):  # Note: file not in same format as fundamentals files
    pass


class CurrentCoupon(AttributeMakerStub):  # Note: file not in same format as fundamentals files
    pass
...
class TraceOLD2(AttributeMaker):
    ...
    def __init__(self, args, event):
        ...
        self._prior_oasspread = {}  # Dict[reclassified_trade_type, prior_oasspread]

    def make_attributes(self, event):
        'return (event_attributes, errs)'
...
class Trace(AttributeMaker):
    def __init__(self, invocation_args):
        ...
        self._supported_reclassified_trade_types = ('B', 'S')
    ...
    def make_attributes(self, event):
        ...
        try:
            ...
            err = 'oasspread string "%s" was not converted to a float' % event.payload['oasspread']
            return None, [err]

        self._oasspreads[reclassified_trade_type].append(oasspread)

        # if we have 2 values for B and S, we can form attributes
        errs = []
        for rtt in self._supported_reclassified_trade_types:
            n = len(self._oasspreads[rtt])
            if n < 2:
                errs.append('have only %d %s oasspreads, not required 2' % (n, rtt))
        if len(errs) > 0:
            return None, errs
        ...
        errs = []
        for rtt in self._supported_reclassified_trade_types:
            if self._oasspreads[rtt][0] == 0.0:
                errs.append('prior %s oasspread is zero' % rtt)
        if len(errs) > 0:
            return None, errs
...
class TestTrace(unittest.TestCase):
    def test_one(self):
        tests = (
            ...
            ((101, 'S'), None),
            ((102, 'S'), None),
            ((103, 'B'), ('B', 103, 103/100, 103 - 100, 102, 102/101, 102 - 101)),
            ((104, 'B'), ('B', 104, 104/103, 104 - 103, 102, 102/101, 102 - 101)),
            ((105, 'B'), ('B', 105, 105/104, 105 - 104, 102, 102/101, 102 - 101)),
            ((106, 'S'), ('S', 105, 105/104, 105 - 104, 106, 106/102, 106 - 102)),
        )
        trace = Trace(None)
        for test in tests:
            event_info, expected = test
            payload = {
                'oasspread': event_info[0],
                'reclassified_trade_type': event_info[1],
                'cusip': 'AJ9',
            }
            event = MockEvent(payload)
            attributes, errs = trace.make_attributes(event)
            if expected is None:
                self.assertTrue(attributes is None)
                self.assertTrue(isinstance(errs, list))
                self.assertTrue(len(errs) > 0)
            else:
                self.assertTrue(errs is None)
                self.assertEqual(attributes['id_trace_reclassified_trade_type'], expected[0])
                self.assertAlmostEqual(attributes['trace_B_oasspread'], expected[1])
                self.assertAlmostEqual(attributes['trace_B_oasspread_divided_by_prior_B'], expected[2])
                self.assertAlmostEqual(attributes['trace_B_oasspread_less_prior_B'], expected[3])
                self.assertAlmostEqual(attributes['trace_S_oasspread'], expected[4])
                self.assertAlmostEqual(attributes['trace_S_oasspread_divided_by_prior_S'], expected[5])
                self.assertAlmostEqual(attributes['trace_S_oasspread_less_prior_S'], expected[6])


class TraceOLD(AttributeMaker):
